...
create policy "Profiles: manage own record" on public.profiles
  for all
  using (id = auth.uid())
...
create policy "Skill nodes: owner goal" on public.skill_tree_nodes
  for all
  using (
    exists (
...
create policy "Sprints: owner goal" on public.sprints
  for all
  using (
    exists (
      select 1 from public.goals g
...
create policy "Sprint tasks: owner sprint" on public.sprint_tasks
  for all
  using (
    exists (
...
create policy "Progress logs: owner" on public.progress_logs
  for all
  using (user_id = auth.uid())
  with check (user_id = auth.uid());
